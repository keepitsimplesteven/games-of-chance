...
{
  "waves": [
    { "id": 0, "tasks": ["1.1", "1.2", "1.3", "1.4", "1.5", "2.1", "2.2", "2.3"] },
    { "id": 1, "tasks": ["3.1", "3.2"] },
    { "id": 2, "tasks": ["4.1", "4.2", "4.3", "6.1", "6.2", "6.3"] },
...
- [x] 3. Derive predetermined matchup winners from placement ordering
  - [x] 3.1 Implement deriveMatchupWinners function
  ...
  - [x] 3.2 Handle bracket advancement simulation in deriveWinners
...
- [x] 4. Implement suppressLoserVictory drive engine handler
  - [x] 4.1 Create suppressLoserVictory function
  ...
  - [x] 4.2 Implement defense-loser turnover suppression
  ...
  - [x] 4.3 Implement defense-loser turnover-on-downs suppression
...
- [x] 5. Integrate suppressLoserVictory into the drive resolution path
  - [x] 5.1 Create resolveLotteryDown wrapper
  ...
  - [x] 5.2 Create createLotteryDriveResolver
  ...
  - [x] 5.3 Add lottery winners module state to PlaycallerPlugin
    ...
    - Note: the room handler now uses schedule-based lookup via `getActiveMatchupsForSchedule` and `bracket.schedule[bracket.currentScheduleIndex]` instead of the old `isComplete(bracket)` gate. The lottery winners check in `resolveMatchupDown()` should work regardless of whether the matchup is main-bracket or consolation — it just needs to look up the matchup ID in the `lotteryWinners` record.
    - Reset lottery winners in `resetPlaycallerState()`
    - _Requirements: 4.8, 4.9, 5.1_

- [x] 6. Add Lottery Mode to session/room configuration
  - [x] 6.1 Extend shared types for lottery mode
  ...
  - [x] 6.2 Wire lottery mode through server room
  ...
  - [x] 6.3 Add Lottery toggle to client landing page
...
- [x] 7. Execute the lottery draw at game start and wire into bracket resolution
  - [x] 7.1 Perform lottery draw during bracket initialization
    - In `room.ts` bracket init block (`if gameType === "playcaller" && roundNumber === 1`): when `progressionMode === "lottery"`:
      1. After `generateBracket(rankedPlayerIds)`, call `generateConsolationRounds(bracket)` to get all possible consolation matchups for deriving winners. Note: at runtime, consolation rounds are generated incrementally by the schedule system — the batch pre-generation here is used only to feed `deriveMatchupWinners` with the full matchup set.
  ...
  - [x] 7.2 Wire lottery resolver for SKIP_GAMEPLAY mode
    - The SKIP_GAMEPLAY path now resolves schedule entries via `PlaycallerPlugin.resolveRound`. For lottery mode, this resolver needs to use the `lotteryMatchupWinners` map instead of `randomResolver` for each matchup.
    - Build a `lotteryResolver: MatchResolver` that returns `lotteryMatchupWinners[matchupId]` for each matchup
    - Use this resolver for both main bracket rounds and consolation rounds resolved through the schedule system
    - _Requirements: 5.1, 5.2, 5.3_

  - [x] 7.3 Handle post-bracket lottery phase transitions
...
- [x] 8. Lottery Reveal screen (client)
  - [x] 8.1 Create LotteryRevealScreen component
  ...
  - [x] 8.2 Add host advance button and phase routing
...
- [x] 9. Draft Pick Selection phase (client + server)
  - [x] 9.1 Create DraftPickScreen component
  ...
  - [x] 9.2 Wire DraftPickScreen into GameView routing
...
- [x] 10. End-to-end integration and statistical validation
  - [x] 10.1 Create lottery integration test suite
  ...
  - [x] 10.2 Statistical validation of lottery draw
  ...
  - [x] 10.3 Consolation bracket validation
...
- The consolation-concurrent-scheduling bugfix (separate spec) has already been applied. That fix introduced schedule-based bracket resolution and `getActiveMatchupsForSchedule`, which this spec's tasks (especially 5.3 and 7.2) depend on being in place.
- Task 7.1 pre-generates consolation rounds in batch solely to feed `deriveMatchupWinners` with the full set of matchup IDs upfront. At runtime, consolation rounds are generated and resolved incrementally through the schedule system — the batch pre-generation is a planning-time operation only and does not affect the live game loop.
