 ...
 * equality), and for any two players A and B where A has a strictly lower
 * session rank number than B, A SHALL appear before B in the spin order.
 *
...
describe("Feature: big-wheel, Property 6: Spin order respects session rank", () => {
  ...
  /**
   * Property 6b: For any two players A and B where A has a strictly lower session
   * rank number than B, A appears before B in the spin order.
   *
   * **Validates: Requirements 3.1, 3.2**
   */
  it("players with strictly lower rank appear before players with higher rank", () => {
    fc.assert(
      fc.property(
        playerIdsArb.chain((ids) =>
          sessionLeaderboardArb(ids).map((lb) => ({ playerIds: ids, leaderboard: lb }))
        ...
        ({ playerIds, leaderboard }) => {
          ...
          // For any pair (A, B) where A has strictly lower rank than B,
          ...
          for (let i = 0; i < result.length; i++) {
            for (let j = i + 1; j < result.length; j++) {
              ...
              // If B has a strictly lower rank than A, the ordering is violated
              expect(rankB).toBeGreaterThanOrEqual(rankA)
            }
          }
        }
      ),
   ...
   * and matches rank ascending.
   *
   * **Validates: Requirements 3.1**
   */
  it("with distinct ranks, output is sorted by rank ascending", () => {
    fc.assert(
      fc.property(
        playerIdsArb.chain((ids) =>
        ...
        ({ playerIds, leaderboard }) => {
          ...
          // Result should be strictly sorted by rank ascending
          for (let i = 0; i < result.length - 1; i++) {
            const rankCurr = rankMap.get(result[i])!
            const rankNext = rankMap.get(result[i + 1])!
            expect(rankCurr).toBeLessThan(rankNext)
          }
        }
      ),
      { numRuns: 100 }
